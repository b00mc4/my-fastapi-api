...
@router.post("/items/{uuid_product}", status_code=201, summary="หยิบสินค้าใส่ตะกร้า")
def add_product_to_cart(body: SaiCart = Depends(), db: Session = Depends(get_db), current_user: UserDATABASE = Depends(get_current_regular_user)):
    return def_cart.add_to_cart_main(body, db, current_user)

@router.put("/items/{uuid_product}", summary="แก้ไขจำนวนสินค้าในตะกร้า")
def update_quantity(body: SaiCart=Depends(), db: Session = Depends(get_db), current_user: UserDATABASE = Depends(get_current_regular_user)):
    return def_cart.edit_quantity(body, db, current_user)
